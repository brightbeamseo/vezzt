-- Flattened current-state signals for market comparison (one row per business).

create or replace view public.business_current_signals
...
select
  ...
  -- Effective local Ahrefs (scope rules applied later in app if needed; view exposes both)
  lsp.parent_seo_id,
  lsp.parent_domain,
...
comment on view public.business_current_signals is
  'One flattened current-state row per business for market comparison. Latest review/seo/map-rank only.';
