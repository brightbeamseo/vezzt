-- ZIP / ZCTA demographic reference data (US Census ACS).

create table if not exists public.zip_code_stats (
  zip_code text primary key,
  population integer,
  households integer,
  housing_units integer,
  owner_occupied_housing_units integer,
  median_household_income numeric,
  median_home_value numeric,
  median_year_structure_built numeric,
  owner_occupied_rate numeric,
  data_source text not null default 'US Census ACS',
...
create index if not exists zip_code_stats_dataset_year_idx
  on public.zip_code_stats (dataset_year);

alter table public.zip_code_stats enable row level security;

drop policy if exists "anon_read_zip_code_stats" on public.zip_code_stats;
create policy "anon_read_zip_code_stats"
  on public.zip_code_stats
  for select
  to anon, authenticated
  using (true);

grant select on public.zip_code_stats to anon, authenticated;

comment on table public.zip_code_stats is
  'ZIP Code Tabulation Area (ZCTA) demographic stats from US Census ACS. Join via businesses.postal_code.';

-- Recreate current-signals view with postal_code + joined ZIP demographics.
